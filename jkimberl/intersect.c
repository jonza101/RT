...
/*   Updated: 2019/10/23 18:37:30 by zjeyne-l         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float		ft_cylinder_intersect(void *mlxx, t_vec3 *origin, t_vec3 *dir, t_obj *obj)
{
    t_mlx *mlx = (t_mlx*)mlxx;
/*
    float k1 = dir->x * dir->x + dir->z * dir->z;
    float k2 = 2.0f * (dir->x * (origin->x - obj->c->x) + dir->z * (origin->z - obj->c->z));
    float k3 = (origin->x - obj->c->x) * (origin->x - obj->c->x) + (origin->z - obj->c->z) * (origin->z - obj->c->z) - obj->radius * obj->radius;
